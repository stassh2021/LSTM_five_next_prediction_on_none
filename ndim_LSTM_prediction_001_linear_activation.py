...
WINDOW_SIZE = 20
steps_predict_forward = 5
act_fcn = 'linear'

print(f'Hyperparams: {WINDOW_SIZE}, {steps_predict_forward},' 
      + f' activation: {act_fcn} ')
...
def sliding_window(df):
    ...
    windows_li = []
    labels_li = []
    for t in range(len(nd_array) - WINDOW_SIZE - steps_predict_forward):
        window = nd_array[t: t+WINDOW_SIZE]
        label = nd_array[t+WINDOW_SIZE + steps_predict_forward]
        windows_li.append(window)
        labels_li.append(label)
    return windows_li, labels_li


def model_constructor(n_features = 6, n_neurons = WINDOW_SIZE):
    
    model = Sequential()
    model.add(LSTM(n_neurons,
              batch_input_shape=(n_batch, None, n_features), stateful=False))
    model.add(Dense(6, activation=act_fcn))
...
def save_struct_and_weights(model):
    ...
    with open(f'models/model_structure_{curr_date_time}_lin_act_{modification}.json', 'w') as f:
        f.write(model_json)
    model.save_weights(f'models/weights_{curr_date_time}_lin_act.h5')
...
csv_file_name_train ='imu_output_recording_12_07_2020_10_53_motion_N.csv'
csv_file_name_test = 'imu_output_recording_09_09_2020_20_23_motion_U_corr_lbls.csv'
# csv_file_name_test = 'imu_output_recording_09_09_2020_20_29_motion_C_corr_lbls.csv'
csv_file_name_test='imu_output_recording_05_11_2020_15_20_motion_N.csv'
...
csv_file_path_train = dir_path + '/' + 'train' + '/' + csv_file_name_train
csv_file_path_test = dir_path + '/' + 'test' + '/' + csv_file_name_test 
...
predictions_li = [np.array([[0, 0, 0, 0, 0, 0]])]*(WINDOW_SIZE + steps_predict_forward) + predictions_li
